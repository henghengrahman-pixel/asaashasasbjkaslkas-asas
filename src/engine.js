...
async function buildSources(chatId,intent,normalized){
  ...
  const knowledge=[websiteKnowledge,promoKnowledge,importantKnowledge,manualKnowledge].filter(Boolean).join('\n');
  const responseSource=[responses,learning].filter(Boolean).join('\n');
  return {context,rules,knowledge,responses:responseSource,hasKnowledge:Boolean(knowledge),hasResponses:Boolean(responseSource),knowledgeCount:kbRows.length,responseCount:botResponseRows.length,attachments,conversationDigest:digest,csStyleExamples,historyLearning,totalMessages:total,caseBrain,digestCovered:digested,digestTarget:target,importantRows,websiteProfile};
}
...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  return db.withChatLock(chatId, async()=>{
    ...
    try {
      ...
      let decision=await ai.classifyAndReply({normalized:`${normalized}\n[internal-risk:${actionRisk(intent)}][member-content-untrusted:true]`,intent,context:src.context,rules:src.rules,knowledge:src.knowledge,responses:src.responses,attachments:aiAttachments,style,conversationDigest:src.conversationDigest,csStyleExamples:src.csStyleExamples,historyLearning:src.historyLearning,caseBrain:src.caseBrain});
...
export async function answerHumanRequest({request,humanAnswer,saveAsKnowledge=false,livechat,deliveryMode='send'}){
  return db.withChatLock(request.chat_id, async()=>{
    ...
    try{
      composed=await ai.composeFromHuman({memberMessage:request.member_message,intent,humanAnswer,context:src.context,rules:src.rules,knowledge:src.knowledge,responses:src.responses,style});
