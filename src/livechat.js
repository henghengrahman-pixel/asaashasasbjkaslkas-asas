...
export class LiveChatClient {
  ...
  inventoryLimitReached(count) {
    return Number(config.lcInventoryLimit||0) > 0 && count >= Number(config.lcInventoryLimit);
  }

  trimInventoryItems(items=[]) {
    const limit=Number(config.lcInventoryLimit||0);
  ...
  mergeInventory(items=[], {replace=false}={}) {
    ...
    if(Number(config.lcInventoryLimit||0)>0 && this._inventoryOrder.length>config.lcInventoryLimit){
      const keep=this._inventoryOrder.slice(0,config.lcInventoryLimit);
      const keepSet=new Set(keep);
      for(const id of this._chatInventory.keys()) if(!keepSet.has(id)) this._chatInventory.delete(id);
      this._inventoryOrder=keep;
    }
  }
