...
export class LiveChatClient {
  ...
  async listChats() {
    ...
    const maxPages = Math.max(1, Math.min(Number(config.lcMaxPages || 50), 200));
    let last;
    for (const baseBody of candidates) {
      try {
        const items=[];
        ...
        let lastData={};
        do {
          const body=pageId ? {...baseBody,page_id:pageId} : {...baseBody};
          const data=await this.call('list_chats',body);
          pagesFetched++;
          lastData=data||{};
          const normalized=this.normalizeChatList(data);
          if(source==='none' && normalized.source!=='none') source=normalized.source;
          rawCount+=normalized.items.length;
          for(const chat of normalized.items){
            const id=String(chat?.id||'').trim();
            if(id){ if(seenIds.has(id)) continue; seenIds.add(id); }
            items.push(chat);
          }
          const next=String(data?.next_page_id||'').trim();
          if(!next) { pageId=null; break; }
          if(seenPageIds.has(next)) throw new Error('LIVECHAT_PAGINATION_LOOP');
          seenPageIds.add(next);
          pageId=next;
        } while(pagesFetched<maxPages);
        if(pageId) {
          const err=new Error(`LIVECHAT_PAGINATION_LIMIT_REACHED:${maxPages}`);
          err.status=502;
          throw err;
        }
        return {
          ...
          _rawCount:rawCount,
          _deduplicatedCount:items.length,
          _duplicateCount:Math.max(0,rawCount-items.length)
        };
      } catch (e) {
        last=e;
  ...
  async test() {
    ...
    return {
      ...
      pagesFetched:Number(data?._pagesFetched||1),
      rawCountAcrossPages:Number(data?._rawCount??items.length),
      deduplicatedChats:Number(data?._deduplicatedCount??items.length),
      duplicateCount:Number(data?._duplicateCount||0),
