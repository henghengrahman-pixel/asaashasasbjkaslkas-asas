...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try {
    ...
    const explicitClosed=rawChats.filter(x=>{const st=livechat.chatState(x);return st.active===false || ['closed','archived'].includes(st.routingStatus);});
    ...
    const seenChatIds=[];
    let newMessages=0, processed=0, fetched=0, unchanged=0, fetchErrors=0, fetchTimeouts=0, bootstrapped=0, reconcileHidden=0, reconcileRestored=0;

    for(const summary of explicitClosed){
      const chatId=String(summary?.id||'').trim();
      if(!chatId) continue;
      const prior=await db.getConversationState(chatId).catch(()=>null);
      if(!prior) continue;
      await db.markConversationEndedWithReason(chatId,{reason:'LIVECHAT_EXPLICIT_CLOSED'}).catch(async e=>{await db.logError('poller','EXPLICIT_CLOSE_RECONCILE_FAILED',e.message,{chatId}).catch(()=>{});});
      reconcileHidden++;
    ...
    for (const [rank, summary] of chats.entries()) {
      ...
      if (!Array.isArray(chat?.threads) || chat.threads.length===0) {
        ...
        catch (e) { fetchErrors++; if(e?.name==='AbortError' || /timeout|aborted/i.test(String(e?.message||''))) fetchTimeouts++; await db.logError('poller','GET_CHAT_FAILED',e.message,{chatId}); continue; }
      }
      if (!chat?.id) continue;
    ...
    lastResult={ok:true,listSource:data?._listSource||'unknown',pagesFetched:Number(data?._pagesFetched||1),rawChats:Number(data?._rawCount??rawChats.length),deduplicatedChats:rawChats.length,activeChats:rawChats.filter(x=>livechat.chatState(x).active!==false).length,closedChats:explicitClosed.length,myActiveChats:chats.length,visibleInbox:seenChatIds.length,fetched,unchanged,fetchErrors,fetchTimeouts,bootstrapped,newMessages,processed,reconcileHidden,reconcileRestored,durationMs,lastSuccessfulFullSync:new Date().toISOString()};
    await db.setIntegrationHealth('livechat_poll',{status:'OK',latencyMs:durationMs,meta:lastResult}).catch(()=>{});
    return lastResult;
  } catch(e){
    lastError=e.message;
