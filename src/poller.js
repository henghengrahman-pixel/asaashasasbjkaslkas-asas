...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  running=true; lastError=null; const syncStarted=Date.now();
  try {
    ...
    // Keep this collection pass intentionally cheap. A single batch touch below prevents
    // thousands of unchanged chats from generating individual PostgreSQL writes every second.
    for (const [rank, summary] of chats.entries()) {
      if (!summary?.id) continue;
      const chatId=String(summary.id);
      seenChatIds.push(chatId);
      workItems.push({rank,summary});
    }
    await db.touchInboxConversations(seenChatIds).catch(()=>{});

    let nextIndex=0;
    const workerCount=Math.max(1,Math.min(Number(config.lcPollConcurrency||6),workItems.length||1));
    const worker=async()=>{
      while(true){
        const index=nextIndex++;
        if(index>=workItems.length) return;
        const {rank,summary}=workItems[index];
        const chatId=String(summary.id);
        const fp=summaryFingerprint(summary);
        const oldFp=summaryFingerprints.get(chatId);

        // Same process + identical provider summary means this chat needs no per-chat DB/API
        // work. inbox_last_seen_at was already refreshed in one batch above.
        ...
        if (!Array.isArray(chat?.threads) || chat.threads.length===0) {
          ...
          catch (e) { fetchErrors++; await db.logError('poller','GET_CHAT_FAILED',e.message,{chatId}); summaryFingerprints.delete(chatId); continue; }
        }
        if (!chat?.id) { summaryFingerprints.delete(chatId); continue; }
    ...
    // Reconcile only after a COMPLETE inventory sweep. During deep pagination the cache is
    // deliberately partial; hiding anything at that point would make busy inboxes blink or lose chats.
    if(data?._inventoryComplete!==false){
      await db.reconcileInboxVisibility(seenChatIds,25);
      const seenSet=new Set(seenChatIds);
      for(const id of summaryFingerprints.keys()) if(!seenSet.has(id)) summaryFingerprints.delete(id);
    }
    lastTick=new Date().toISOString();
    lastResult={ok:true,listSource:data?._listSource||'unknown',rawChats:rawChats.length,chats:chats.length,fetched,unchanged,fetchErrors,bootstrapped,newMessages,processed,inventorySize:Number(data?._inventorySize||rawChats.length),inventoryComplete:data?._inventoryComplete!==false,deepSyncRunning:Boolean(data?._deepSyncRunning),pageCount:Number(data?._pageCount||1),concurrency:workerCount,durationMs:Date.now()-syncStarted};
...
export async function stopPoller({waitMs=5000}={}){ if(timer) clearTimeout(timer); timer=null; const until=Date.now()+waitMs; while(running&&Date.now()<until)await new Promise(r=>setTimeout(r,50)); return !running; }
